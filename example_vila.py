...
def prune_attention_layer(multihead_attn_layer, pruned_heads):
    ...
    pruned_o_weights = o_weights.view(num_heads, head_dim, embed_dim)[keep_heads].reshape(embed_dim, len(keep_heads) * head_dim)

    
    # pruned_q_weights = q_weights.view(embed_dim, num_heads, head_dim)[:,keep_heads,:].reshape(len(keep_heads) * head_dim, embed_dim)
    # pruned_k_weights = k_weights.view(embed_dim, num_heads, head_dim)[:,keep_heads,:].reshape(len(keep_heads) * head_dim, embed_dim)
    # pruned_v_weights = v_weights.view(embed_dim, num_heads, head_dim)[:,keep_heads,:].reshape(len(keep_heads) * head_dim, embed_dim)
    # pruned_o_weights = o_weights.view(num_heads, head_dim, embed_dim)[keep_heads].reshape(embed_dim, len(keep_heads) * head_dim)

    # Assign pruned weights and biases back to the layer
    multihead_attn_layer.q_proj.weight.data = pruned_q_weights
    multihead_attn_layer.k_proj.weight.data = pruned_k_weights
    multihead_attn_layer.v_proj.weight.data = pruned_v_weights
    multihead_attn_layer.o_proj.weight.data = pruned_o_weights
    
    multihead_attn_layer.num_heads = remaining_heads
    multihead_attn_layer.num_key_value_heads = remaining_heads # fix
...
def prune_model_layers(model, prune_dict):
    ...
    for name, module in model.named_modules():
        ...
        if name in prune_dict:
            ...
            print(f"layer {name} remains {module.num_heads} heads")
