...
class LlamaFlashAttention2(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        layer_name = f"model.layers.{self.layer_idx}.self_attn"
        mask_heads = self.config.mask_heads_dict.get(layer_name, None)
        print(f"found mask_heads: {mask_heads} in {layer_name}")
        if mask_heads is not None:
            # 对 attn_output 的指定 heads 进行 mask 操作
            mask = torch.ones_like(attn_output)
            for head in mask_heads:
                head_start = head * self.head_dim
                head_end = (head + 1) * self.head_dim
                # print(f"setting [:, :, {head_start}:{head_end}] = 0")
                mask[:, :, head_start:head_end] = 0 # 将指定的 head 的输出置为 0
